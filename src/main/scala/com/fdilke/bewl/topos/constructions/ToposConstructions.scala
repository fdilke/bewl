...
/*
  class ToposOfActions[M <: ~](val monoid: Monoid[M]) extends Topos with Wrappings[
    ...
    import monoid.multiply
    import monoid.Action
    import monoid.ActionPreArrow
    ...
    object ActionDot {
      def apply[
        ...
        wrap(new monoid.Action(actionCarrier, actionMultiply))
    ...
    class ActionArrow[
      ...
      override def preEqualizer[
        ...
        new ActionDot[A, AA](
          new monoid.Action(thunkedEqualizer, source.action.actionMultiply(_, _)),
          source.↔
