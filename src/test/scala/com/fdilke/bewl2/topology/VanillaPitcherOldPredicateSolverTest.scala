...
import com.fdilke.bewl2.topology.VanillaPitcherOldPredicateSolver.{solvePitcher, solveSeq}
...
class VanillaPitcherOldPredicateSolverTest extends AnyFunSpec {
  describe("The predicate solver can act on maps") {
    it("detects immediate success for the rubberstamp predicate on an empty map") {
      def rubberstampPredicate(calendar: VanillaPitcherOld[StrontiumDog]): Boolean =
        true
    ...
    it("detects immediate failure for the Dr No predicate") {
      def drNoPredicate(calendar: VanillaPitcherOld[StrontiumDog]): Boolean =
        false
    ...
    it("solves in one go for the Johnny-as-head predicate") {
      def johnnyOnWed(calendar: VanillaPitcherOld[StrontiumDog]): Boolean =
    ...
    it("solves for the Tuesday-same-as-Wednesday predicate on an empty map") {
      def tueSameWed(calendar: VanillaPitcherOld[StrontiumDog]): Boolean =
    ...
    it("can generate a seq, but not a pitcher for uninhabited types") {
      def rubberstampPredicate(calendar: VanillaPitcherOld[Impossibility]): Boolean =
        true
